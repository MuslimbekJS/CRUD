...
const lolation = document.getElementById('lolation');
...
const lolationEdit = document.getElementById('lolationEdit');
const birthDateEdit = document.getElementById('dateEdit');
const postypeEdit = document.getElementById("postypeEdit");
...
function displayStudents(students) {
    ...
    students.forEach((student) => {
      ...
      <tr >
        ...
        <th>${student.lolation}</th>
        <th>${student.birthDate}</th>
        <th>${student.postype}</th>
        <th>${student.salary}</th>
        <td>${student.group}</td>
        <td>${student.doesWork ? 'Ha' : "Yo'q"}</td>
        <td>
          <button class="btn btn-sm btn-warning" data-bs-toggle="modal"
          data-bs-target="#editModal" onclick="handleEdit(${student.id
            })">Edit</button>
...
function handleEdit(studentId) {
    ...
    lolationEdit.value = studentEdit.lolation;
    birthDateEdit.value = studentEdit.birthDate;
    postypeEdit.value = studentEdit.postype;
...
btnUpdate.addEventListener('click', function (e) {
    ...
    let studentUpdated = {
        ...
        lolation: lolationEdit.value,
        birthDate: birthDateEdit.value,
        postype: postypeEdit.value,
...
filter.addEventListener('change', function (e) {
    ...
    let students = JSON.parse(localStorage.getItem('students'));
    let newStudents = value === 'All' ? students : students.filter((student) => student.group === value);
    displayStudents(newStudents);
});


filterLoc.addEventListener('change', function (e) {
    let value = e.target.value;
    let students = JSON.parse(localStorage.getItem('students'));
    let newStudents = value === 'All' ? students : students.filter((student) => student.lolation === value);
    displayStudents(newStudents);
});
